package com.danielaboros.appandroid.network

import com.danielaboros.androidapp.objects.MuseumObject
...
class MuseumRepositoryImpl() : MuseumRepository {
    ...
    private suspend fun fetchObjectDetails(ids: List<Int>): List<MuseumObject?> =
    ...
    override suspend fun getObjects(query: String, page: Int): Result<List<MuseumObject>> =
        withContext(Dispatchers.IO) {
            runCatching {
    ...
    override suspend fun getObjects(page: Int): Result<List<MuseumObject>> =
        withContext(Dispatchers.IO) {
            runCatching {
    ...
    override suspend fun getObject(objectId: Int): Result<MuseumObject?> =
        withContext(Dispatchers.IO) {
            runCatching {
                val response = api.getObject(objectId)
